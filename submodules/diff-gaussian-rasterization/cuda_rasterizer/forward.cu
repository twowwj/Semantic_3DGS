	...
	bool antialiasing)
{
	...
	if (colors_precomp == nullptr)
	{
		...
		features[idx * C + 0] = result.x;
		features[idx * C + 1] = result.y;
		features[idx * C + 2] = result.z;
	}
	else
	{
		features[idx * C + 0] = colors_precomp[idx * 3 + 0];
	...
	float* __restrict__ invdepth)
{
	...
	if (inside)
	{
		...
		for (int ch = 0; ch < CHANNELS; ch++)
			out_color[ch * H * W + pix_id] = C[ch] + (ch < 3 ? T * bg_color[ch] : 0.0f);
